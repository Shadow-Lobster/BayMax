...
#!/user/bin/perl
##############
source ./modules/ipLookup
source ./modules/urban
source ./modules/seen
##############
. bot.properties
input=".bot.cfg"
...
tail -f $input | openssl s_client -connect $server:6697 -CApath /etc/ssl/certs/ | while read res
do
  # log the session
...
#### nick change for bot
	*"@nick"*)
	...
	;;
### private msg to anyone or anything or any channel
	*"@pm"*)
	 who=$(echo "$res" | perl -pe "s/:(.*)\!.*@.*/\1/")
	 msg=$(echo "$res" | sed "s/^.*://")
	 bts=$(echo "$msg" | sed "s/[^ ]* //")
...
##### sents private messeges recieved by bot to specified user
	*"PRIVMSG $nick"*)
	...
	;;
##### looks up details of ip.
	*":@whois"*)
	 msg=$(echo "$res" | sed "s/^.*://")
	 bts=$(echo "$msg" | sed "s/[^ ]* //")
	 ip=$(echo "$bts" | sed "s/ .*//")
	 from=$(echo "$res" | perl -pe "s/.*PRIVMSG (.*[#]?([a-zA-Z]|\-)*) :.*/\1/")
	 ret=$(ipDetailsLookup $ip)
	 echo "PRIVMSG $from :$ret" >> $input
	;;
#### urban dictionary
	*":@urban"*)
	 msg=$(echo "$res" | sed "s/^.* ://")
	 bts=$(echo "$msg" | sed "s/[^ ]* //")
	 flags=$(echo "$bts" | perl -pe "s/(.*?)[^ ].*/\1/")
	 from=$(echo "$res" | perl -pe "s/.*PRIVMSG (.*[#]?([a-zA-Z]|\-)*) :.*/\1/")
	 definition=$(urbanDefLookup $bts)
	 example=$(urbanExLookup $bts)
	 echo "PRIVMSG $from : $definition" >> $input
	 echo "PRIVMSG $from : example: $example" >> $input
	 echo "PRIVMSG $from : $flags" >> $input
	;;
	*":@seen "*)
...
#####
	*"-users"*)
	 ...
	 if [ "$who" = "Akuma" ]
	 then
	 	echo "NAMES #lobby" >> $input
	 	read res
	 	case "$res" in
	 		*"irc.evilcorp.ga"*)
	 	 	names=$(echo "$res" | perl -pe "s/:irc.evilcorp.ga .* :(.*)/\1/")
		 	echo "PRIVMSG Akuma :Users that are currently on #lobby are: $names" >> $input
			;;
	 	esac
	 else echo "PRIVMSG #lobby :This command is only for discord users" >> $input
	 fi
	;;
#### makes the bot quit
	*":@quit"*)
	 who=$(echo "$res" | perl -pe "s/:(.*)\!.*@.*/\1/")
	 if [ "$who" = "Shadow_Lobster" ]
	  then echo "PRIVMSG $channel :adios" >> $input
	       echo "QUIT someone is satisfied with my care" >> $input
	 elif [ "$who" = "shadowlobster" ]
	  then echo "PRIVMSG $channel :adios" >> $input
	       echo "QUIT someone is satisfied with my care" >> $input
	 else
	  echo "PRIVMSG $who you are not the boss of me" >> $input
	 fi
	;;
##### Test and Junk stuff
#	*"Shadow_Lobster"*)
